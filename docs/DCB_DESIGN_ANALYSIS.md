...
| 字段 | 设置者 | 用途 |
|------|--------|------|
| `domain_id` | 系统分配 | 全局唯一域标识符，用于 DMA 访问控制、安全组绑定 |
| `ctrlblock_size` | 父域设置 | 控制块大小（用于校验） |

**分析**：
- `domain_id`：用于 DMA 访问控制、安全组绑定等
...
当前设计：`security_group` 存储安全组 handle

**决策**：
- 保留 `security_group`：绑定安全组
...
- 0x30: saved_psr（ASCEND 保存的程序状态寄存器）
...
| SECDOMAIN | 0x04 | CREATE, DESTROY, BIND, UNBIND, GET_ID, SET_ENCRYPTION, ADD_ACCESSOR, REMOVE_ACCESSOR, FORCE_DESTROY, GET_HANDLE | 安全组管理 |
...
```python
# 通过 handle 访问安全组实例
security_group = dcb[0x1C]  # handle
...
3. **安全组系统加密区域**：是否需要 DCB 字段还是完全通过内存管理器管理？